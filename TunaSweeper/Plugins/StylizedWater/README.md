...
`Enable Terrain-Following Runup` builds a second non-colliding procedural mesh on the sampled terrain. It lets the animated waterline and foam render above dry shoreline terrain instead of being hidden below it. `Terrain Overlay Offset` keeps that mesh slightly above the ground to avoid z-fighting. Rebuild and bake after sculpting the shoreline.
