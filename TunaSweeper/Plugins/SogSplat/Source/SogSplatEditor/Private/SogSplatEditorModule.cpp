...
UMaterialInterface* SogSplatEditorUtils::EnsureDefaultSogMaterial()
{
	...
	UMaterialExpressionScalarParameter* EdgeSoftness = NewObject<UMaterialExpressionScalarParameter>(Material);
	EdgeSoftness->Material = Material;
	EdgeSoftness->ParameterName = TEXT("EdgeSoftness");
	EdgeSoftness->DefaultValue = 0.35f;
	EdgeSoftness->MaterialExpressionEditorX = -680;
	EdgeSoftness->MaterialExpressionEditorY = 580;
	Material->GetExpressionCollection().AddExpression(EdgeSoftness);
	...
	SoftEllipseOpacity->Code =
		...
		TEXT("float feather = saturate((1.0f - radiusSq) / max(EdgeSoftness, 0.0001f));\n")
		TEXT("float smoothFeather = feather * feather * (3.0f - 2.0f * feather);\n")
		TEXT("return Alpha * smoothFeather;");
	...
	EdgeInput.InputName = TEXT("EdgeSoftness");
	EdgeInput.Input.Connect(0, EdgeSoftness);
...
private:
	...
	TObjectPtr<USogActorFactory> SogActorFactory;
};
