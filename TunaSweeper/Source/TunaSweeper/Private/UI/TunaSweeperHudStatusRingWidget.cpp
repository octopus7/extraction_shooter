...
void UTunaSweeperHudStatusRingWidget::NativeConstruct()
{
	Super::NativeConstruct();
	SetVisibility(ESlateVisibility::HitTestInvisible);
}

void UTunaSweeperHudStatusRingWidget::SetStatusPercent(float InStatusPercent)
{
	StatusPercent = FMath::Clamp(InStatusPercent, 0.0f, 1.0f);
	InvalidateLayoutAndVolatility();
}

void UTunaSweeperHudStatusRingWidget::SetRingColors(const FLinearColor& InTrackColor, const FLinearColor& InFillColor)
{
...
int32 UTunaSweeperHudStatusRingWidget::NativePaint(
	...
	bool bParentEnabled) const
{
	...
	PaintVertices.Reset();
	PaintIndices.Reset();
	PaintVertices.Reserve(388);
	PaintIndices.Reserve(576);
	...
	if (StatusPercent > 0.0f)
	{
		AddRingSegment(
			StartAngleRadians,
			GaugeSweepRadians * StatusPercent,
			InnerRadius,
			OuterRadius,
			FillColor,
			AccumulatedRenderTransform,
...
void UTunaSweeperHudStatusRingWidget::AddRingSegment(
	...
	TArray<SlateIndex>& OutIndices) const
{
	...
	const int32 SegmentCount = FMath::Clamp(FMath::CeilToInt((ClampedSweep / (2.0f * PI)) * 96.0f), 2, 96);
