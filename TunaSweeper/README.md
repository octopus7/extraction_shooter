...
- Steam 데모/본편은 `BatchScripts/PackageTunaSweeperSteamDemoWin64.bat`와 `BatchScripts/PackageTunaSweeperSteamFullWin64.bat`, STOVE 데모/본편은 `BatchScripts/PackageTunaSweeperStoveDemoWin64.bat`와 `BatchScripts/PackageTunaSweeperStoveFullWin64.bat`로 패키징한다. 각 스크립트는 기본 Shipping이며 필요하면 첫 번째 인자로 `Development`를 지정할 수 있다.
