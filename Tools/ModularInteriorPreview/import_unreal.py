"""UE 5.7 import/reload. Saves ONLY /Game/Environment/ModularInteriorPreview."""
...
atlas=texture('T_MI_Atlas',True);mask=texture('T_MI_DirtMask',False)
materials={}
for name,s in m['materials'].items():
...
for label,tint,band in [('Light',[1.2,1.2,1.16],[1,1,1]),('Dark',[.29,.35,.40],[1,1,1]),('Managed',[.97,1.05,1.03],[.17,.48,.50])]:
    name='MI_MI_Concrete_'+label
    mi=unreal.load_asset(f'{DEST}/Materials/{name}')
    if not verify:
        if not mi:mi=assets.create_asset(name,DEST+'/Materials',unreal.MaterialInstanceConstant,unreal.MaterialInstanceConstantFactoryNew())
        lib.set_material_instance_parent(mi,materials['M_MI_Concrete'])
        lib.set_material_instance_vector_parameter_value(mi,'Tint',unreal.LinearColor(*tint,1))
        lib.set_material_instance_vector_parameter_value(mi,'BandColor',unreal.LinearColor(*band,1));save(mi)
    assert mi;materials[name]=mi
...
report={'engine':unreal.SystemLibrary.get_engine_version(),'mode':'reload' if verify else 'import','assets':[],
