"""UE 5.7 import/reload. Saves ONLY /Game/Environment/ExtractionMarkers."""
...
atlas=texture('T_EM_Atlas',True)
mask=unreal.load_asset(BASE+'/Textures/T_MI_DirtMask');assert mask
materials={}
for name,s in {'M_EM_Surface':{'tint':[1,1,1.],'roughness':.52,'metallic':.25}}.items():
    ...
    if not verify:
        ...
        if name.endswith('_LED'):
            ...
            strength=scalar('DirtStrength',.25,0)
...
for name,rgb,emission in [('M_EM_Green',[.025,.65,.10],2)]:
    mat=assets.create_asset(name,DEST+'/Materials',unreal.Material,unreal.MaterialFactoryNew()) if not unreal.EditorAssetLibrary.does_asset_exist(DEST+'/Materials/'+name) else unreal.load_asset(DEST+'/Materials/'+name)
    lib.delete_all_material_expressions(mat);mat.set_editor_property('blend_mode',unreal.BlendMode.BLEND_OPAQUE)
    c=color('Color',rgb);prop(c,'RGB','BASE_COLOR');prop(mul(c,'RGB',scalar('Emission',emission),''),'','EMISSIVE_COLOR');prop(scalar('Roughness',.35),'','ROUGHNESS');lib.recompile_material(mat);save(mat);materials[name]=mat
    off=assets.create_asset('MI_EM_GreenOff',DEST+'/Materials',unreal.MaterialInstanceConstant,unreal.MaterialInstanceConstantFactoryNew()) if not unreal.EditorAssetLibrary.does_asset_exist(DEST+'/Materials/MI_EM_GreenOff') else unreal.load_asset(DEST+'/Materials/MI_EM_GreenOff')
    lib.set_material_instance_parent(off,mat);lib.set_material_instance_scalar_parameter_value(off,'Emission',0);save(off)
off=assets.create_asset('MI_EM_AmberOff',DEST+'/Materials',unreal.MaterialInstanceConstant,unreal.MaterialInstanceConstantFactoryNew()) if not unreal.EditorAssetLibrary.does_asset_exist(DEST+'/Materials/MI_EM_AmberOff') else unreal.load_asset(DEST+'/Materials/MI_EM_AmberOff')
lib.set_material_instance_parent(off,unreal.load_asset('/Game/Environment/ModularInteriorExpansion/Materials/M_MI_ExpansionAmber'));lib.set_material_instance_scalar_parameter_value(off,'Emission',0);save(off)
if not verify:
    for e in m['assets']:
        if e['reused_ue']:continue
        ...
        for i,slot in enumerate(mesh.get_editor_property('static_materials')):
            ...
            mesh.set_material(i,materials[n])
...
# Verification runs on a later editor tick after asynchronous texture compilation.
