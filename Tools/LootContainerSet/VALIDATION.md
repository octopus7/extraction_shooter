...
원본 커밋: `464414c6`, UE 애셋 커밋: `8a3ef8ce`. Blender 4.5.12 LTS 및 UE 5.7.4로 검증했다. 기존 C++를 수정하지 않은 자산 작업이며, 로컬의 호환 에디터 바이너리를 사용했다.
...
UE 커밋 직후 일회성 `import_once.py`를 제거했다. 제거 후 새 UE 프로세스의 저장 애셋 재로드에서 20,456개 검사가 다시 통과했으며 프로세스 종료 코드는 0이다. 기본 실행기는 읽기 전용 검증기를 실행한다. 프로젝트 C++/플러그인/시작 설정에 생성기 진입점이나 전용 의존성을 추가하지 않았다.
