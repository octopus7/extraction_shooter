...
TILES = {
    'Pink':(0,0,.12,.38),'Ivory':(1,0,.08,.32),'Oak':(2,0,0,.65),'Walnut':(3,0,0,.68),
    'Steel':(0,1,.72,.32),'Iron':(1,1,.55,.58),'Brass':(2,1,.72,.32),'Glass':(3,1,.15,.19),
    'Gingham':(0,2,0,.9),'Rug':(1,2,0,.98),'Lace':(2,2,0,.95),'Floral':(3,2,0,.52),
    'Sage':(0,3,.1,.42),'Terracotta':(1,3,0,.83),'Turquoise':(2,3,.05,.32),'Linen':(3,3,0,.92),
}
...
def init(label):
    ...
    atlas=bpy.data.images.load(str(OUT/'Textures/T_InteriorAdditions_Atlas.png'))
    atlas.pack();atlas.filepath='//Textures/T_InteriorAdditions_Atlas.png'
    for key,(_,_,metal,rough) in TILES.items():
        m=bpy.data.materials.new('M_InteriorAdditions_'+key);m.use_nodes=True
...
def begin(short_name,target_size_cm,reference=''):
    ...
    parts=[];colls=[]
    current={'name':'SM_InteriorAdditions_'+short_name,'target_size_cm':list(target_size_cm),'category':category,'reference':reference}
...
def finish(o,name,mat='Oak',bevel=1,smooth=False):
    ...
    if bevel:
        mod=o.modifiers.new('Soft crafted edge','BEVEL');mod.width=bevel/100;mod.segments=3
...
def complete():
    manifest={'category':category,'axis_contract':'X width, -Y front, Z up; floor-centred pivots; authoring cm, Blender m, FBX unit scale', 'materials':{k:{'name':mats[k].name,'metallic':v[2],'roughness':v[3]} for k,v in TILES.items()},'assets':entries,'total_triangles':sum(e['triangles'] for e in entries)}
    ...
    bpy.ops.wm.save_as_mainfile(filepath=str(OUT/f'InteriorAdditions_{category}.blend'))
