...
- 별도 확인용 맵에 29종을 실제 배율 1로 전시하고 명칭·조명·카메라를 저장했다. 조명은 `Showcase_Key` 디렉셔널 1개이며 Source Angle은 45도다. 중복 디렉셔널 경고를 없애기 위해 `Showcase_Fill`은 제거했다. 충돌체는 총 39개, 치수 최대 오차는 0.00002cm 미만이며 최초 검증에서 기존 Agit 에셋과 기존 맵 142개의 해시가 유지된다.
