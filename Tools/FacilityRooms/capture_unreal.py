...
Launch only after importing the FacilityRooms assets. Example arguments for a
...
Optional environment variables:
  FACILITY_CAPTURE_LEVEL       Saved /Game/Environment/FacilityRooms/Maps/... map
  FACILITY_CAPTURE_CAMERAS     Comma-separated actor labels; default both views
  FACILITY_CAPTURE_OUTPUT      Output directory; default SourceArt/.../Previews
  FACILITY_CAPTURE_WARMUP      Minimum rendered warmup seconds per camera (15)
  FACILITY_CAPTURE_FRAMES      Minimum Slate warmup ticks per camera (90)
  FACILITY_CAPTURE_TIMEOUT     Overall callback timeout seconds (600)
...
MAP_ROOT = '/Game/Environment/FacilityRooms/Maps/'
DEFAULT_LEVEL = MAP_ROOT + 'L_Facility_ControlFloor02'
DEFAULT_CAMERAS = 'Facility_ReviewCamera,Facility_InteriorCamera'
WIDTH, HEIGHT = 1600, 900


def package_hashes():
    """Read only this collection's content files to detect accidental saves."""
    folder = Path(unreal.Paths.project_content_dir()).resolve() / 'Environment/FacilityRooms'
    return {
...
class FacilityCapture:
    def __init__(self):
        ...
        self.level_path = os.environ.get('FACILITY_CAPTURE_LEVEL', DEFAULT_LEVEL)
        ...
        self.labels = [s.strip() for s in os.environ.get(
            'FACILITY_CAPTURE_CAMERAS', DEFAULT_CAMERAS).split(',') if s.strip()]
        allowed = {'Facility_ReviewCamera', 'Facility_InteriorCamera'}
        ...
        self.output = Path(os.environ.get('FACILITY_CAPTURE_OUTPUT', str(SOURCE / 'Previews'))).resolve()
        self.output.mkdir(parents=True, exist_ok=True)
        self.warmup_seconds = max(5.0, float(os.environ.get('FACILITY_CAPTURE_WARMUP', '15')))
        self.warmup_frames = max(30, int(os.environ.get('FACILITY_CAPTURE_FRAMES', '90')))
        self.timeout = max(60.0, float(os.environ.get('FACILITY_CAPTURE_TIMEOUT', '600')))
        ...
        if not self.baseline:
            raise RuntimeError('FacilityRooms content does not exist in this project')
        self.report = {
            'passed': False,
    ...
    def transition(self, state):
        ...
        self.frames = 0
        unreal.log(f'FACILITY_CAPTURE_STAGE {state}')

    def start(self):
        unreal.EditorPythonScripting.set_keep_python_script_alive(True)
        self.handle = unreal.register_slate_post_tick_callback(self.tick)
        unreal.log(f'FACILITY_CAPTURE_STARTED {self.level_path}')
    ...
    def _tick(self, delta_seconds):
        ...
        try:
            ...
            if self.state == 'wait_viewport':
                ...
                unreal.log('FACILITY_CAPTURE_IMAGE_READY ' + json.dumps(evidence))
                self.task = None
                self.camera_index += 1
    ...
    def select_camera(self):
        ...
        unreal.log(f'FACILITY_CAPTURE_CAMERA {label}')
        self.transition('warmup')

    def request_screenshot(self):
        label = self.labels[self.camera_index]
        suffix = label.removeprefix('Facility_').removesuffix('Camera')
        map_name = self.level_path.rsplit('/', 1)[-1].removeprefix('L_Facility_')
    ...
    def finish(self, error=None):
        ...
        try:
            ...
            (unreal.log if self.report['passed'] else unreal.log_error)(
                ('FACILITY_VIEWPORT_CAPTURE_PASSED ' if self.report['passed'] else 'FACILITY_VIEWPORT_CAPTURE_FAILED ')
                + str(report_path))
            if error:
                unreal.log_error(error)
        except Exception:
            unreal.log_error('FACILITY_VIEWPORT_CAPTURE_REPORT_FAILED ' + traceback.format_exc())
        finally:
...
def main():
    ...
    try:
        ...
        unreal.log_error('FACILITY_VIEWPORT_CAPTURE_START_FAILED ' + traceback.format_exc())
        unreal.EditorPythonScripting.set_keep_python_script_alive(False)
        unreal.SystemLibrary.quit_editor()
        raise
