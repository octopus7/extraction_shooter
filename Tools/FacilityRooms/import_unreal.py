...
def create_forest_material():
    ...
    if mat is None:
        mat=ASSETS.create_asset('M_FacilityRooms_ForestImpostor',DEST+'/Materials',unreal.Material,unreal.MaterialFactoryNew())
...
def create_master(texture):
    ...
    if master is not None:
        ...
        return master
    master = ASSETS.create_asset("M_FacilityRooms_Master", DEST + "/Materials", unreal.Material, unreal.MaterialFactoryNew())
    assert isinstance(master, unreal.Material)
    lib = unreal.MaterialEditingLibrary
...
def create_levels():
    for level in verify.read_layout()["levels"]:
        ...
        for entry in level["placements"]:
            ...
            comp.set_static_mesh(unreal.load_asset(f"{DEST}/Meshes/{entry['mesh']}"))
            comp.set_collision_profile_name("BlockAll")
            comp.set_collision_enabled(unreal.CollisionEnabled.QUERY_AND_PHYSICS)
        player=spawn(world,unreal.PlayerStart,level["player_start"])
        player.set_actor_label("Facility_PlayerStart")
        ...
        for entry in level["placements"]:
            if entry["mesh"].endswith(("WallLamp","AtticLamp")):
                xyz=entry["location_cm"]
                light=spawn(world,unreal.PointLight,[xyz[0],xyz[1]+25,xyz[2]+35])
                light.set_actor_label("Practical_"+entry["label"])
                comp=light.get_component_by_class(unreal.PointLightComponent)
                comp.set_mobility(unreal.ComponentMobility.MOVABLE)
                comp.set_intensity(350 if attic else 900);comp.set_attenuation_radius(350)
        ...
        camera.set_actor_label("Facility_ReviewCamera")
        ...
        if attic:
            ...
            camera=spawn(world,unreal.CameraActor,spec['location_cm']);camera.set_actor_label('Facility_InteriorCamera')
...
def main():
    ...
    if os.environ.get("FACILITY_LAYOUT_ONLY")!="1":
    ...
    unreal.log("FACILITY_UE_IMPORT_VALIDATION_PASSED")
