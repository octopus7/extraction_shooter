...
for level in layout['levels']:
    ...
    library={}
    for family in ['Shell','Console','Decor']:
        with bpy.data.libraries.load(str(SOURCE/f'MoleControlV2_{family}.blend'),link=False) as (available,loaded):
            loaded.objects=[n for n in available.objects if n.startswith('SM_MoleControlV2_')]
        for obj in loaded.objects:library[obj.name]=obj
    with bpy.data.libraries.load(str(SOURCE.parent/'FacilityRooms/FacilityRooms_Architecture.blend'),link=False) as (available,loaded):
        loaded.objects=['SM_FacilityRooms_Ladder300']
    for obj in loaded.objects:library[obj.name]=obj
    ...
    for spec in level['placements']:
        ...
        if spec['zone'] in {'Architecture','VaultEnd'} and ('Wall' in spec['mesh'] or 'VaultEnd' in spec['mesh']):
    ...
    if attic:
        ...
        mat=bpy.data.materials.new('M_MoleControlV2_ForestImpostor');mat.use_nodes=True
        nodes=mat.node_tree.nodes;nodes.clear()
        out=nodes.new('ShaderNodeOutputMaterial');em=nodes.new('ShaderNodeEmission');tex=nodes.new('ShaderNodeTexImage')
        tex.image=bpy.data.images.load(str(SOURCE/'Textures/T_MoleControlV2_Forest.png'));tex.image.pack()
        tex.image.filepath='//Textures/T_MoleControlV2_Forest.png'
        ...
        spec=level['window_light'];data=bpy.data.lights.new('WindowSunBeam','SPOT');data.energy=500
    ...
    for label,offset,color,power in [('Key',(-6,5,11),(1,.78,.54) if attic else (1,.90,.76),light_power),
        ...
        data.color=color
    for spec in level['placements']:
        if spec['mesh'].endswith('RoseTableLamp'):
            data=bpy.data.lights.new('Practical','POINT');data.energy=35;data.color=(1,.72,.47);data.shadow_soft_size=.15
            light=bpy.data.objects.new('REVIEW_'+spec['label'],data);scene.collection.objects.link(light)
            light.location=point(spec['location_cm'])+Vector((0,0,.23))
    for name,xyz,power in [('CeilingBounce',[0,0,625],100),('MapTask',[0,173,522],20)]:
        data=bpy.data.lights.new(name,'POINT');data.energy=power;data.color=(1,.80,.62);data.shadow_soft_size=.4
        light=bpy.data.objects.new('REVIEW_'+name,data);scene.collection.objects.link(light);light.location=point(xyz)
    ...
    print('MOLE_V2_SCENE_RENDERED',level['name'],len(level['placements']))
