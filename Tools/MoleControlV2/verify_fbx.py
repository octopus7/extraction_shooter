...
def verify_open_space(entry, mesh, colliders):
    ...
    name = entry['name'].removeprefix('SM_MoleControlV2_')
    rays = []
    if name == 'ArchedDoorway200':
        rays = [([x, -40, z], [x, 40, z]) for x in (-40, 0, 40) for z in (10, 100, 160)]
        rays.append(([0, -40, 210], [0, 40, 210]))
    elif name == 'OakHatch200':
        rays = [([x, y, -15], [x, y, 35]) for x in (-40, 0, 40) for y in (-40, 0, 40)]
    elif name == 'StoneWindowWall200':
        # Keep probes inside the 100 x 110 cm aperture and away from the central
        # vertical/horizontal muntins, which are legitimate window geometry.
        rays = [([x, -45, z], [x, 45, z]) for x in (-40, -20, 20, 40) for z in (110, 135, 175, 200)]
...
def main():
    ...
    for entry in entries:
        try:
            ...
            print('MOLE_V2_FBX_ASSET_FAILED', entry['name'], str(error))
    ...
    print('MOLE_V2_FBX_VALIDATION', json.dumps({key: report[key] for key in (
