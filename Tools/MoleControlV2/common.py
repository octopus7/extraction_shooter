...
OUT = ROOT / 'TunaSweeper/SourceArt/Environment/MoleControlV2'
TILES = {
    'Stone':(0,0,0,.88),'Oak':(1,0,0,.64),'Brass':(2,0,.72,.38),'Iron':(3,0,.65,.52),
    'Rose':(0,1,0,.86),'Cream':(1,1,0,.82),'Leather':(2,1,0,.65),'Paper':(3,1,0,.92),
    'Map':(0,2,0,.72),'Dial':(1,2,.15,.4),'Amber':(2,2,0,.35),'Green':(3,2,0,.35),
    'BookRed':(0,3,0,.75),'BookGreen':(1,3,0,.75),'Ceramic':(2,3,0,.3),'Leaf':(3,3,0,.7),
}
EMISSIONS={'Map':.35,'Amber':2.0,'Green':1.5}
mats={}; parts=[]; colls=[]; objects=[]; entries=[]; current={}; category=''; extra_material_specs={}
...
def init(label):
    ...
    atlas=bpy.data.images.load(str(OUT/'Textures/T_MoleControlV2_Atlas.png'))
    atlas.pack();atlas.filepath='//Textures/T_MoleControlV2_Atlas.png'
    for key,(_,_,metal,rough) in TILES.items():
        m=bpy.data.materials.new('M_MoleControlV2_'+key);m.use_nodes=True
        ...
        if key in EMISSIONS:
            ...
            bs.inputs['Emission Strength'].default_value=EMISSIONS[key]
        mats[key]=m

def begin(short_name,target_size_cm,reference=''):
    global current,parts,colls
    parts=[];colls=[]
    current={'name':'SM_MoleControlV2_'+short_name,'target_size_cm':list(target_size_cm),'category':category,'reference':reference}

def finish(o,name,mat='Oak',bevel=1,smooth=False):
...
def box(name,loc,size,mat='Oak',bevel=1):
...
def cylinder(name,loc,radius,depth,mat='Cream',axis=(0,0,1),vertices=32,bevel=.3):
...
def sphere(name,loc,size,mat='Cream'):
...
def ring(name,loc,major,minor,mat='Brass',axis=(0,0,1),scale=(1,1,1)):
...
def pipe(name,points,radius,mat='Brass'):
...
def surface(name,verts_cm,faces,mat='Cream',bevel=0,smooth=False):
...
def lathe(name,profile,loc=(0,0,0),mat='Cream',scale=(1,1,1),segments=48):
...
def end():
    ...
    assert export_coll, 'Every model needs its own collision boxes'
...
def complete():
    manifest={'category':category,'axis_contract':'Blender X width/-Y front/Z up -> UE X width/+Y front/Z up; floor-centred pivots, cm', 'materials':{k:{'name':mats[k].name,'metallic':v[2],'roughness':v[3],'emission':EMISSIONS.get(k,0.0)} for k,v in TILES.items()},'assets':entries,'total_triangles':sum(e['triangles'] for e in entries)}
    manifest['materials'].update(extra_material_specs)
    ...
    columns=2 if category=='Shell' else 3;spacing=7 if category=='Shell' else 4.5 if category=='Console' else 3.5
    ...
    d=bpy.data.cameras.new('ReviewCamera');cam=bpy.data.objects.new('PREVIEW_Camera',d);scene.collection.objects.link(cam);cam.location=target+Vector((6,-12,15));cam.rotation_euler=(target-cam.location).to_track_quat('-Z','Y').to_euler();d.type='ORTHO';d.ortho_scale=25 if category=='Shell' else 15;scene.camera=cam
    bpy.ops.wm.save_as_mainfile(filepath=str(OUT/f'MoleControlV2_{category}.blend'))
