...
- 엔딩은 다음 지역 이동이 아니라 **루나와 캔봇의 일상 안정화 엔딩**이다.
- 최종보스는 본편 기준의 **최종보스**다.
...
| M04 | 캔봇의 체크리스트 | 메인 / 캐릭터 소개 | M03 | 클립보드 x1, 볼펜 x1, 반쯤 남은 건전지 x2 | 퀘스트 목록 안내, 숲길 탐색 유도 |
...
- **카테고리:** 메인 / 튜토리얼
- **내용:** 루나가 동면 장치에서 깨어난다. 캔봇이 현재 벙커 상태와 외부 탐색 필요성을 알려준다.
- **요구 아이템:** 없음
...
```text
캔봇: 환기 필터 상태가 좋지 않습니다. 벙커 공기 질이 기준치 아래입니다.
루나: 그럼 먼저 고쳐야 해?
캔봇: 우선 필터부터 확인하면 됩니다. 그리고… 화장실도 언젠가는 고쳐야 합니다.
루나: 화장실?
캔봇: 불편하니까요.
루나: 나 안드로이드인데?
캔봇: 섭취 기능이 있으면 배출 문제도 따라옵니다.
루나: 그건 너무 현실적인데…
캔봇: 현실은 대체로 불편합니다.
```
...
- **카테고리:** 메인 / 캔봇 성격 소개
- **내용:** 캔봇이 쓸데없이 꼼꼼한 생존 체크리스트를 작성하려 한다.
...
- **카테고리:** 메인 / 엔딩
- **내용:** 벙커에서 마지막 식탁을 준비한다. 루나와 캔봇의 일상이 안정되었음을 보여주며 엔딩에 진입한다.
...
| S09 | 캔봇의 취향 | 생활 / 캐릭터 대사 | M04 완료 | 귀여운 자석 x3, 병뚜껑 컬렉션 x1 | 캔봇 대사 추가, 벙커 장식 변화 |
...
| S17 | 캔봇의 분류법 | 생활 / 보관함 개그 | M04 완료 | 색깔 스티커 x3, 빈 파일철 x2, 이름표 라벨 x1 | 보관함 대사 추가 |
| S18 | 엔딩 후: 오늘도 참치 | 엔딩 후 / 반복 플레이 | M20 완료 | 찌그러진 참치캔 x3, 깨끗한 포크 x1 | 엔딩 후 일상 대사 |
| S19 | 엔딩 후: 별거 아닌 하루 | 엔딩 후 / 후일담 | M20 완료 | 작은 꽃병 x1, 오래된 잡지 x1, 멀쩡한 식탁보 x1 | 루나/캔봇 후일담 대사 |
...
| 클립보드 | 캔봇 체크리스트 | 낡은 갈색 클립보드. 금속 클립이 녹슬고 종이 자국이 남아 있음. |
...
| 아이템명 | 용도 | 외양 묘사 |
|---|---|---|
| 귀여운 자석 | 캔봇 취향 | 냉장고 자석. 작은 참치캔, 별, 오리 모양 등. 밝은 색이지만 칠이 벗겨짐. |
| 병뚜껑 컬렉션 | 캔봇 취향 | 여러 병뚜껑이 실에 꿰어진 묶음. 색이 다양하고 일부는 납작하게 눌림. |
...
- **용도:** 엔딩 최종 조건
- **설명 후보:** 평범한 참치와는 다르다. 적어도 캔봇은 그렇게 주장한다.
