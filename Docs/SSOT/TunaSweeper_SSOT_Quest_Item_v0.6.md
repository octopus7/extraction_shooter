...
- 수납량은 **장착한 가방**으로 늘린다.
- 기본 수납량은 40칸이며, 1~4단계 가방 장착 시 각각 50/60/80/100칸을 사용한다.
- 가방 아이템과 용량의 원본은 `TunaSweeper/Content/Data/ItemTable.json`의 `equipment.slot.backpack`, `inventory_slot_capacity`다.
- 캐릭터의 기본 수납량을 재료 소비로 영구 증가시키는 기능과 퀘스트는 사용하지 않는다.
...
- 가방은 획득한 아이템을 장비 슬롯에 장착해 사용한다.
- 가방을 통한 수납량 증가는 메인 퀘스트 완료나 영구 확장 재료 소비를 조건으로 하지 않는다.
...
```text
두더지: 환기 필터가 많이 더러워. 이대로는 너한테도 안 좋아.
루나: 그럼 먼저 고쳐야 해?
두더지: 응. 필터부터 보자. 그리고… 화장실도 언젠가는 고쳐야 해.
루나: 화장실?
두더지: 나도 불편하거든.
루나: 나 안드로이드인데?
두더지: 먹을 수 있으면 그 뒤 문제도 따라오는 거야.
루나: 그건 너무 현실적인데…
두더지: 현실은 대체로 불편해.
```
...
## 7. 선택 / 편의 / 하우징 / 부수 퀘스트 17개

S01~S03의 영구 수납 확장 퀘스트는 폐기했다. 나머지 퀘스트의 기존 ID는 유지한다.
...
5. 최종보스 명칭과 위치
6. 고급보안구역의 구체 구조
7. 자판기 폭발물 가격/재고/반복 구매 여부
8. 보스 처치와 고급참치 획득의 관계
9. 엔딩 연출 세부 대사
...
- 철지난 NFT와 전용 재료를 소비하는 영구 수납 확장, S01~S03 퀘스트 및 대화 (2026-09-06: 미구현 기획을 제거하고 장착 가방 기준으로 정정)
- 강제 시설복구1 / 시설복구2
- 화장실 수리 퀘스트
- 보급창고 필수 진행 구역화
- 포탈을 메인퀘스트에서 안내하는 구조
