...
타이틀 전용 `ABP_LunaMk2_Title`의 컴포넌트 공간 포즈 연결 순서:

```text
타이틀 Sequence Evaluator → Local To Component → Title Head Look
...
목표가 없거나 비활성화된 경우와 일반 인게임 SkeletalMeshComponent에서는 머리 보정 노드가 입력 포즈를 그대로 통과시킨다. 인게임 캐릭터는 기존 `ABP_LunaMk2`와 물리 설정을 유지한다. 타이틀 ABP는 C의 마지막 0.5초 동안 `HeadLookAlpha`를 0에서 1로 올려 뒷모습과 커서 추적을 연결한다. 본이 없는 LOD에서는 보정을 건너뛴다.
