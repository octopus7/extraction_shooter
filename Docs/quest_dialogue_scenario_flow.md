...
flowchart TD
    ...
    MOLE["Mole / Facility NPC"] -->|ResolveQuestId| QS
    MOLE -->|Quest notice visibility| QS
...
CSV 또는 JSON 로딩이 실패하면 `RegisterFallbackQuest()`가 최소 대체 퀘스트 `quest_first_outing`을 등록한다. 이 대체 퀘스트는 `provider.mole` 제공자, `BunkerMap`에서 `RaidMap`으로 이동하는 목표, 100 코인 보상을 가진다.
...
Mole의 퀘스트 상호작용은 현재 생성자에서 별도 `ObjectiveEventId`를 설정하지 않는다. Mole 퀘스트 체인은 주로 제공자 ID `provider.mole`와 퀘스트 패널 수락/보상 흐름으로 연결된다.
...
- `quest_first_outing`: `provider.mole`, `level_travel`, `BunkerMap`에서 `RaidMap` 이동, 100 코인 보상
- `quest_lumberjack_first_kill`: `provider.mole`, 선행 `quest_first_outing`, `enemy_killed`로 `enemy.lumberjack` 처치, 150 코인 및 `housing_workbench` 해금
...
sequenceDiagram
    ...
    PC->>PC: QueueScenarioTrigger(level_entered)
    PC->>Dialogue: 활성 플레이버 JSON에서 조건을 만족한 대화 재생
    Dialogue->>GI: 완료 시 데이터에 지정된 completion_flag 저장
...
`BunkerMap` 로드 후 `TunaSweeperPlayerController::ShowBunkerEntryFadeIfNeeded()`가 `CompletePendingScenarioBunkerEntryIfNeeded()`를 호출한다. pending flag가 있으면 이를 `CompletedScenarioFlags`에 저장하고 검은 화면 페이드 인을 재생한다. 이어서 `level_entered` 트리거를 `UTunaSweeperScenarioSubsystem`에 전달한다. 서브시스템은 활성 플레이버의 `ScenarioDefinitions.json`에서 레벨, 필수/차단 플래그, 선택적 퀘스트 상태와 완료 여부를 평가하고 `ScenarioTextStrings.csv`의 현지화된 대사를 반환한다.
...
5. Demo는 오프닝 없이 벙커로 직접 진입하고 Demo JSON의 `level_entered` 정의를 선택한다. Main은 오프닝을 거친 뒤 저장된 `scenario.opening.awakening` 조건을 만족할 때 Main JSON의 첫 벙커 대화를 선택한다.
6. Demo 대화 완료 시 `dialogue.demo.toilet_intro`, Main 대화 완료 시 `dialogue.main.bunker_intro`가 저장된다. 이후 자동 재생은 각 completion flag 때문에 생략된다.
7. Mole 일반 대화 상호작용은 `interaction.mole` 트리거로 같은 플레이버의 조건에 맞는 시나리오를 강제 재생할 수 있다. 이미 완료된 경우에는 completion flag를 다시 저장하지 않는다.
...
11. 이후 Mole notice는 첫 퀘스트가 `RewardAvailable` 상태이므로 다시 표시될 수 있고, 보상 수령 후 다음 provider 퀘스트 조건이 열리면 새 notice 조건을 평가한다.

## 시나리오 플래그, 퀘스트 상태, 월드 상태의 차이

| 구분 | 저장 위치 | 예시 | 용도 | 퀘스트 목표와의 관계 |
| --- | --- | --- | --- | --- |
| 시나리오/대화 플래그 | `UTunaSweeperSaveGame::CompletedScenarioFlags` | `scenario.opening.awakening`, `dialogue.demo.toilet_intro`, `dialogue.main.bunker_intro` | 오프닝 완료, 플레이버별 자동 대화 재생 여부, 초기 레벨 결정 | 현재 전용 목표 타입 없음. 퀘스트 진행을 직접 증가시키지 않음 |
...
- Mole 퀘스트가 interaction objective로 진행되지 않는다면 현재 Mole 퀘스트 컴포넌트에는 별도 objective event ID가 설정되어 있지 않다는 점을 먼저 확인한다.
...
- Main 첫 벙커 대화가 나오지 않으면 Main 시나리오 JSON의 `required_completed_flags`와 슬롯의 `scenario.opening.awakening`을 확인한다.
- 시나리오 문구가 비거나 재생되지 않으면 활성 플레이버의 `ScenarioDefinitions.json` 문자열 키와 `ScenarioTextStrings.csv` 행이 일치하는지 확인한다.
