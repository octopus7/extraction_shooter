...
`sandbag_cover` rows create `ATunaSweeperSandbagCoverActor` through `BP_SandbagCover`. The current default cover uses `box_extent=[37.5, 160.0, 45.0]`, half of the previous prototype size. The actor blocks pawns and projectiles, takes projectile damage, and starts a scripted collapse when health reaches zero. Collision is disabled at collapse start, so the cover is immediately passable while the visible bags slide down and spread out for a short hold before the actor destroys itself.

The visible stack uses repeated `/Game/Interaction/SM_Sandbag_LowPoly` static mesh components rather than runtime procedural boxes. The generated mesh shares each ring vertex and assigns matching smooth vertex normals to duplicate vertex instances, so normal-based outline expansion does not split the bag into separate face shells. When the player is within `passthrough_radius` of the cover bounds, each visible sandbag component gets `/Game/Interaction/M_SandbagCover_OverlayOutline` as its overlay material. That material expands the rendered shell along `VertexNormalWS` by `OutlineThickness`, masks the visible-facing side with `TwoSidedSign`, and renders a bright unlit inverted-hull outline through the real sandbag mesh shape instead of a dummy proxy. Player-fired projectiles ignore that cover for the shot. Enemy projectiles still collide with and damage the cover until collapse starts, so shots blocked by the sandbags do not continue into the player.
