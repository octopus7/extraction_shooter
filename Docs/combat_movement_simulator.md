...
- 화면 오른쪽은 월드 `+Y`다.
- 플레이어는 WASD로 이동하고, 마우스 방향으로 조준하며, 좌클릭으로 사격한다.
- 장애물은 회전 가능한 직사각형이다.
...
- 총기 적은 선호 거리 유지, 후퇴, 측면 이동, 직사 화선 탐색을 수행한다.
...
- 플레이어 반경, 이동 속도, 체력, 투사체 속도, 피해량, 연사 간격
- 적 반경, 체력, 근접/총기 이동 속도
- 추적 거리, 근접 공격 거리, 근접 선딜/후딜, 근접 피해량
...
- 총기 적이 선호 거리 안에서 너무 많이 흔들리거나 너무 정지해 보이지 않는지.
...
- 상태 enum은 `Idle`, `Approach`, `HoldRange`, `Strafe`, `Retreat`, `SeekLineOfFire`, `AttackCommit`, `Recover`를 기본으로 가져간다.
- 거리는 cm 단위 그대로 이식한다.
...
- 엄폐 후보 점수화는 아직 없고, 직사 화선 탐색은 측면 이동 기반이다.
