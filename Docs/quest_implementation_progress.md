...
- `Docs/SSOT/area_unlocking.md` 기준 본편 지역 개방 퀘스트 데이터를 추가한다.
- 서쪽 우회로, 영구 개방 문, 맵 경로 언락 1~3, 검문초소, 시설 복구 2, 워프포인트, 보스 구역, 벙커 완전 복구 상태를 퀘스트/월드 진행 데이터로 연결한다.
- 교관 외 퀘스트 제공자가 필요하면 provider 데이터를 확장한다.
