...
| 에셋 | 길이 | 동작 |
| --- | --- | --- |
| `AS_LunaMk2_Title_A` | 4초 | 상체를 살짝 앞으로 숙이고 양팔을 몸 옆 대각선 아래로 벌린 기본 포즈. 팔꿈치와 손가락은 부드럽게 굽히고 손목은 팔의 흐름에 맞춘다. 발을 고정하고 흉곽·어깨에 미세한 호흡을 준 루프. |
| `AS_LunaMk2_Title_B` | 4초 | 왼다리로 체중을 옮기고 오른발을 약간 앞으로·바깥으로 내민 호흡 루프. |
| `AS_LunaMk2_Title_C` | 3초 | 화면 왼쪽을 보는 약 90도 측면에서 짧게 멈춘 뒤, 발을 들어 디디며 정면 A 포즈로 전환. 2026-09-21 사용자 첨부 구도의 좌우 반전 방향으로 수정했으며, 최초 C와 반대로 회전한다. |
| `AS_LunaMk2_Title_AtoB` | 1.6초 | 오른발을 들어 B 자세로 옮기는 연결 모션. |
| `AS_LunaMk2_Title_BtoA` | 1.6초 | 오른발을 들어 A 자세로 되돌리는 연결 모션. |

모두 30fps, `SKM_LunaMk2_Skeleton` 사용. 루트 이동은 없고 루트 모션 추출은 비활성화했다. C는 Blender Z 회전 -90→0도, UE root yaw +90→0도로 베이크되어 있으므로 액터를 같은 각도로 중복 회전시키지 않는다. A/B는 루프 재생, C와 연결 모션은 한 번 재생한다.

A/B 첫 프레임은 호흡의 같은 위상이다. 연결 모션 시작·끝은 해당 루프의 0초 포즈와 일치한다. 랜덤 전환 시 루프 경계에서 연결 모션을 시작하면 포즈가 정확히 맞으며, 임의 시점 전환은 별도 블렌딩이 필요하다.

2026-09-21 팔 자세 수정: 등 뒤 중앙에서 겹치는 손을 풀고 A/B/C/AtoB/BtoA 모두 양팔을 옆으로 벌리는 자세로 통일했다. `verify_unreal.py`는 C의 회전을 제거한 좌표에서도 손이 각각 자기 쪽에 유지되고 손목 사이가 45cm 이상 떨어지는지 검사한다. `Previews/OpenArms_Front.png`, `OpenArms_Side.png`, `OpenArms_Back.png`로 앞·옆·뒤 실루엣을 확인할 수 있다.
...
별도 타이틀 ABP와 연결 에셋·일회성 생성기는 `a0580c7c`에 함께 커밋했다. ABP 생성기는 후속 정리에서 제거하고 실행용 AnimInstance 및 읽기 전용 검증만 유지한다.
